...
# # ! CLK
# def CLK(csv_file,secret):
#     fields = [
#         StringSpec('id', FieldHashingProperties(comparator=NgramComparison(2), strategy=BitsPerFeatureStrategy(300)))
#     ]
#     schema = Schema(fields, 1024)
#     clks = clk.generate_clk_from_csv(open(csv_file,'r'), secret, schema)
#     return [item.tobytes()  for item in clks]

def intersect_shuffle(_lists):
    start_intersection = time.time()
    _intersect = list(set(_lists[0]).intersection(*_lists[1:]))
    np.save("./dataset/IdList_intersection.npy",np.array(_intersect))
    ...
    print("--inter:{}--".format(end_intersection-start_intersection))
    # print(_shuffles)
    return _shuffles
...
def run_mpc_sdap(
    ...
    if rank != world_size-1:
        ...
        scatter_list_seeds = [(torch.tensor([random.getrandbits(63) for _ in range(_dataset.features.size(1))]).type(torch.int64) if i != rank else torch.zeros(_dataset.features.size(1)).type(torch.int64) ) for i in range(world_size-1)]
        # print('===========Rank {} scatter=============\n{}\n'.format(rank,scatter_list_seeds))

        # Store the ramdom share from other parties
        gather_list_seeds = [torch.zeros(gather_list_dataset_size[i].item()).type(torch.int64) for i in range(world_size-1)]
        ...
        # ! CLK
        # _id_share = CLK('./dataset/IdList_{}.csv'.format(rank),'secret')
        # print(_id_share)
        ...
        wait_for_intersection_time = time.time()
        ...
        _shuffles = torch.zeros(world_size-1,_intersection_size.item()).type(torch.int64)
        ...
        gather_list_seeds_TTP = [torch.zeros(_intersection_size.item()).type(torch.int64) for _ in range(world_size-1)]
        ...
        for k in range(world_size-1):
            ...
            if k == rank:
                ...
                if k == label_provider:
                    ...
                    # print("---------\nRank{}'s party{}'s _share:{}\n----------------".format(rank,k,sdap_labels_share))
                continue
        ...
        if rank ==0:
            ...
            print('===========Rank {} sdap_features=============\n{}\n'.format(rank,sdap_features_share.shape)) 
        dist.reduce(sdap_labels_share,dst=0,group=usr_group)
        if rank ==0:
            # print('===========Rank {} sdap_features=============\n{}\n'.format(rank,sdap_features_share)) 
            print('===========Rank {} sdap_labels=============\n{}\n'.format(rank,sdap_labels_share.shape)) 
        if rank ==0:
            with open('./dataset/stat.csv'.format(i), 'a', newline='') as csvfile:
                writer  = csv.writer(csvfile)
                writer.writerow(['sdap',world_size-1,_dataset.features.size(1),sdap_features_share.size(0),sdap_features_share.size(1),wait_for_intersection_time-start,intersection_time-wait_for_intersection_time,end_time-intersection_time,end_time-start])
        ...
        gather_list_ids.pop()
        gather_list_ids = [ ids for ids in gather_list_ids]
        # print('===========Rank {} gather ids=============\n{}\n'.format(rank,gather_list_ids)) 
        ...
        print("--rank:{}--inter:{}--".format(rank,end_intersection-start_intersection))
        # print(gather_list_ids)
        # print(len(_shuffles))
        ...
        # the zero sharing
        # ! random!!!
        # scatter_list_zerosharing_seeds = [[((torch.arange(_intersection_size.item())+1)*1.0 * (j  + 1) * (i + j + 1) if (j != i and j != world_size-1) else torch.zeros(_intersection_size.item()) ) for j in range(world_size)] for i in range(world_size-1)]
        scatter_list_zerosharing_seeds = [[((torch.tensor([random.getrandbits(63) for _ in range(_intersection_size.item())]).type(torch.int64)) if (j != i and j != world_size-1) else torch.zeros(_intersection_size.item()).type(torch.int64) ) for j in range(world_size)] for i in range(world_size-1)]
        ...
        print("Compute the random sharing (share - PRG(zerosharing seeds))")
        # print(scatter_list_zerosharing_seeds)
        for k in range(world_size-1):
            # if k !=0: continue
            for i in range(gather_list_features_share[k].size(1)):
                for j in range(world_size-1):
                    # print(scatter_list_zerosharing_seeds[k])
                    if k == j: 
                        continue
                    generator = torch.Generator()
                    # print("---------------{}------------{}-------{}--{}--{}-------------".format(rank,scatter_list_zerosharing_seeds[k][j][i],k,j,i))
                    generator.manual_seed(scatter_list_zerosharing_seeds[k][j][i].item())
                    # _share = generate_random_ring_element(size=(3,1), generator=generator)
                    # print('===========Rank {} example{} party {}_share=============\n{}\n'.format(rank,i,j,_share))
                    _share = generate_random_ring_element(gather_list_features_share[k][::,i].size(), generator=generator)
                    # if k==2 and j==1:
                    #     print("---------\nTTP Rank{}'s party{}'s sub _share:{}\n----------------".format(j,k,_share))
                    gather_list_features_share[k][::,i] -= _share
                    # print('===========Rank {} example{} party {}_share=============\n{}\n'.format(k,i,j,_share))
                    if k == label_provider:
                        _share = generate_random_ring_element(_labels_share[::,i].size(), generator=generator)
                        _labels_share[::,i] -= _share
