...
#     STSenders = []
#     for _ in range(3):
#         STSenders.append(Sender(size=sub_examples, permute=all_indices, ios_threads = 4))
...
#         port = port+rank * 31 + dset_rank * 73 + input_dim * 109,
#         port_mode=False,
#         num_threads = num_threads)
    
# def STrecv_thread(args):
#     sub_examples, client_rank,server_rank, port,num_threads,dset_rank, input_dim = args
#     if client_rank == dset_rank:
#         return
#     STRecver = Receiver(ios_threads = 4)
#     # a_s[dset_rank, :,
#         # input_dim], b_s[dset_rank, :, input_dim] = node.STrecv(
#     STrecv( STRecver = STRecver,
#             size=sub_examples,
#             dset_rank=dset_rank,
#             sender=server_rank,
#             tag= client_rank * 31 + dset_rank * 73 + input_dim * 109,
#             port = port+ client_rank * 31 + dset_rank * 73 + input_dim * 109,
#             port_mode=False,
#             num_threads = num_threads)
...
#         #     shared_STSenders = manager.list()
#         #     for STSender in STSenders:
#         #         shared_STSenders.append(Sender(size=3, permute=[0,1,2], p=p, ios_threads = 4))
...
#         task_args = [(sub_examples, client_rank,server_rank, port,num_threads, dset_rank, input_dim)
#                         for input_dim in range(n)
#                         for dset_rank in range(client_size)
#                         ]
#         with concurrent.futures.ProcessPoolExecutor() as executor:
...
port = 20000
# dest = range(size)
permute = list(range(size-1, -1, -1))

def rec_th(args):
    sessionHint, Sip=args
    receiver = SSS.OSNReceiver(ios_threads = 4)
    return receiver.run(size=size, sessionHint=sessionHint, p=p, Sip=Sip, num_threads=num_threads)

def sen_th(args):
    sessionHint, Sip=args
    sender = SSS.OSNSender(size=size, dest=permute,p=p,ios_threads = 4)
    return sender.run(size=size, sessionHint=sessionHint, p=p, Sip=Sip, num_threads=num_threads)
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        if rank == 0:
            task_args = [(str(i), Sip+":"+str(i+port)) for i in range(10)]
            executor.map(rec_th,  task_args)
            # print(a[0][-1])
            # print(a[1][-1])
        else:
            task_args = [(str(i), Sip+":"+str(i+port)) for i in range(10)]
            executor.map(sen_th,  task_args)
            # print(a[0])
